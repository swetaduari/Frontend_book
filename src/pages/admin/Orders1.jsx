                ...
                <div className="mb-6 rounded-[28px] border border-slate-200 bg-white/90 p-5 shadow-[0_20px_50px_rgba(15,23,42,0.08)] sm:p-6">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <p className="mt-2 text-sm text-slate-500">Track customer purchases and current order status.</p>
                        </div>
                        <Link
                ...
                <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_20px_50px_rgba(15,23,42,0.08)]">
                    <div className="overflow-x-auto">
                        <table className="min-w-[800px] w-full text-left text-sm">
                            ...
                            <tbody>
                                {orders.map((order) => (
                                    <tr key={order.orderId} className="border-b border-slate-200 bg-white hover:bg-slate-50">
                                        <td className="p-3 font-semibold text-slate-700">{order.orderId}</td>
                                        <td className="p-3">{order.user?.firstName} {order.user?.lastName}</td>
                                        <td className="p-3">{order.user?.email}</td>
                                        <td className="p-3 font-semibold text-emerald-600">₹ {order.totalAmount}</td>
                                        <td className="p-3">
